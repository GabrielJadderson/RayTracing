...
public class ConcurrentRayTracer
{
	...
	public static ExecutorService executorService = Executors.newFixedThreadPool(Runtime.getRuntime().availableProcessors() * 2); //fix thread pool number
	...
	public ConcurrentRayTracer(Camera camera, Main main)
	{
		...
		IntStream.range(0, (HEIGHT * WIDTH)).parallel().forEach(x -> //parallel is a good idea, but doesn't affect run time too much.
		{
	...
	private void rayTraceAsync(Main main, CountDownLatch countDownLatch, int pixelNumber, AtomicInteger c)
	{
		...
		Globals.renderer.pixelMap.put(pixelNumber, p);
		//Globals.renderer.globalQueue.add(p);
