...
import core.concurrency.RenderingSubdivider;
import core.gui.GUI;
...
public class Main
{
	...
	public static void main(String[] args) throws IOException, InterruptedException
	{
		...
		Vector3D origin = new Vector3D(-2.8, 1.2, 1.0);
		...
		Globals.renderer.keepRendering = true;
		//ConcurrentRayTracer concurrentRayTracer = new ConcurrentRayTracer(camera, main); //HOLY SHIT
		//ConcurrentSubDivider concurrentSubDivider = new ConcurrentSubDivider(10, camera, Globals.boundingList, globalLight);
		//concurrentSubDivider = null;
		
		RenderingSubdivider renderingSubdivider = new RenderingSubdivider(3, 3, WIDTH * HEIGHT, camera, boundingList, globalLight);
		//System.exit(0);
		Globals.renderer.finalizePicture();
		Globals.renderer.keepRendering = false;
		//rayTrace(camera);
	...
	public static void createAndAddObjects() throws FileNotFoundException, IOException
	{
		...
		//loadOBJ("bunny.obj", 0, 0, -1, new Material(new Vector3D(Math.random(), Math.random(), Math.random())));
		//loadOBJ("suzanne.obj", 2.7, 0.35, -10, new Material(new Vector3D(Math.random(), Math.random(), Math.random())));
		//loadOBJ("wt_teapot.obj", 0.01, 0.01, -10, new Material(new Vector3D(Math.random(), Math.random(), Math.random())));
		
		//loadOBJ("suzanne.obj", -2.6, 2.4, -10, new Material(new Vector3D(Math.random(), Math.random(), Math.random())));
		//loadOBJ("dragon.obj", 2.6, 2.3, -10, new Material(new Vector3D(Math.random(), Math.random(), Math.random())));
		//loadOBJ("wt_teapot.obj", 0.01, 2.3, -10, new Material(new Vector3D(Math.random(), Math.random(), Math.random())));
		
		//loadOBJ("dragon.obj", -2.6, -2.7, -10, new Material(new Vector3D(Math.random(), Math.random(), Math.random())));
		//loadOBJ("wt_teapot.obj", 2.6, -2.7, -10, new Material(new Vector3D(Math.random(), Math.random(), Math.random())));
		//loadOBJ("bunny.obj", 0.01, -2.7, -10, new Material(new Vector3D(Math.random(), Math.random(), Math.random())));
		...
		for (int i = -5; i < 5; i++)
		{
			for (int j = -5; j < 5; j++)
			{
				Sphere onecirkle = new Sphere(new Vector3D((double) i / 3.67, 0, (double) j / 3.67), 0.05 + (Math.random() / 15), new Material(new Vector3D(Math.random(), Math.random(), Math.random())));
...
//        shapeList.add(mySphere_2);
	...
	public static void loadOBJ(String filename, double x, double y, double z, Material mat) throws FileNotFoundException, IOException
	{
